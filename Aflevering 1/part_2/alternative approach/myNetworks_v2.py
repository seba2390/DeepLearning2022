    ...
    def train_network(self, train_dataloader, validation_dataloader,
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for _, (x_batch, y_batch) in enumerate(train_dataloader):
                ...
                with torch.no_grad():
                    if self.device.type != 'cpu':
                        _batch_train_acc += torch.sum(_y_hat == y_batch).item() / train_dataloader.batch_size
                    else:
                        _batch_train_acc += torch.sum(_y_hat == y_batch) / train_dataloader.batch_size
            ...
            for _, (x_batch, y_batch) in enumerate(validation_dataloader):
                ...
                with torch.no_grad():
                    if self.device.type != 'cpu':
                        _batch_validation_acc += torch.sum(_y_hat == y_batch).item() / train_dataloader.batch_size
                    else:
                        _batch_validation_acc += torch.sum(_y_hat == y_batch) / train_dataloader.batch_size
...
class NeuralNet4(torch.nn.Module):
